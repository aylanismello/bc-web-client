...
class App extends Component {
  ...
  componentWillUpdate(nextProps, nextState) {
    ...
    if (!_.isEqual(nextState.trackFilters, this.state.trackFilters)) {
      ...
      this.fetchTracks(nextState.trackFilters, paginate, nextState.playFirstNewTrackOnLoad);
    }
  ...
  fetchTracks(trackFilters, paginate = false, playFirstNewTrackOnLoad = false) {
    ...
    if (
      !trackFilters.soundcloud_user_id &&
      ((trackFilters.sort_type === 'trending' && trackFilters.track_type === 0) ||
        trackFilters.track_type === 1 ||
        trackFilters.track_type === 2 ||
        trackFilters.sort_type === 'obscure')
    ) {
  ...
  fetchSoundcloudUser(id, track_type) {
    ...
    axios.get(`${baseUrl}/soundcloud_users/${id}`).then(results => {
      const {
 soundcloud_user, handles, location, associated_users
} = results.data.data;
            ...
            >
              ...
              <SideMenu
                ...
                clickedOnMenuItem={menuItem => {
                  window.amplitude.getInstance().logEvent('Click on Menu Item', {
                    menuItem
                  });
                ...
                <Route
                  ...
                  render={() => (
                    <Home
                      ...
                      setError={error => this.setState({ error, success: null })}
                      tracks={this.state.tracks}
                      setSuccess={success => this.setState({ success, error: null })}
