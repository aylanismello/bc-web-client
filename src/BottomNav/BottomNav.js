  ...
  if (playing) {
    tracks.forEach((track, idx) => {
      if (JSON.stringify(track.track) === JSON.stringify(currentTrack)) {
        nextTrack = tracks[idx+1] ? tracks[idx+1].track : tracks[0].track;
        prevTrack = tracks[idx-1] ? tracks[idx-1].track : tracks[tracks.length - 1].track;
      }
    })
  }
  return (
    ...
    >
      ...
      <NextButton
        ...
        onClick={() => {
          if (scPlayer.audio.currentTime < 3) {
      ...
      { playing && <NextButton
        ...
        onClick={() => togglePlay(nextTrack.id, false, true)}
      /> }
